...
- `GET /api/funds`：列出绑定 Virtual Account 的 Fund 配置和最近采样时间。
- `POST /api/funds`：创建或更新 Fund 配置，包含目标币种和净值采样间隔。
- `POST /api/funds/sample?fund_id=...`：立即对指定 Fund 采样一次净值。
- `GET /api/fund-nav?fund_id=...`：读取 Fund 净值快照历史。
...
Fund 迁移自 Yuan `app-fund` 的定期权益记录思路，但在 1Exchange 中直接绑定一个本地 Virtual Account。Virtual Account 负责多账户线性合成；Fund 后台任务按 `poll_interval_seconds` 定期组合该虚拟账户，把 positions 按目标币种估值后写入 SQLite `fund_nav_snapshots` 表。默认采样间隔为 600 秒，后台每 60 秒检查一次到期 Fund。

Fund 净值当前使用 position `valuation`，并通过内置汇率图转换到 `target_currency`。无法转换的持仓不会强行计入 0 以外的估值，会在快照中记录 `unpriced_positions` 数量。
