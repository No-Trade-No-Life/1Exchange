...
- `IAccountInfo`：账户快照，包含资金、持仓、订单和生成时间。
- `IAccountMoney`：账户资金，包含权益、余额、浮动盈亏、可用和占用资金。
...
- `GET /api/assets`：列出资产统计。
- `GET /api/products`：列出交易产品。

当前 `accounts`、`positions`、`assets`、`products` 已固定标准响应模型，但真实数据拉取仍待交易所 Adapter 接入。
...
3. 建立 Yuan 语义对应的账户、资金、持仓、订单、产品模型。
...
2. 将账户、持仓、资产和产品 API 接入 Adapter Registry。
3. 增加 Credential 更新、删除和本地加密策略。
